...
static int ReadLineBlocking(char* buf, int max_len) {
    ...
    while (idx < max_len - 1) {
        int c = getchar();
        if (c == EOF) { vTaskDelay(pdMS_TO_TICKS(1)); continue; }
...
static void TfLiteSetup() {
    ...
    if (kUseContext && kUseFovea) { input_fov_tensor = interpreter->input(0); input_ctx_tensor = interpreter->input(1); }
    else if (kUseContext) { input_ctx_tensor = interpreter->input(0); }
    else { input_fov_tensor = interpreter->input(0); }
...
static int ParseFloatsIntoFloat(float* dst, int offset, int max, const char* csv) {
    ...
    for (char* tok = strtok_r(tmp, ",", &saveptr); tok && (offset + written) < max; tok = strtok_r(nullptr, ",", &saveptr)) {
...
static void ProtocolLoop() {
    ...
    if (len <= 0) { vTaskDelay(pdMS_TO_TICKS(10)); return; }
    ...
    if (state == RecvState::WAIT_HEADER) {
        ...
        if (sscanf(line_buf, "S %d %d", &cN, &fN) == 2) {
            ...
            ctx_expected = ce; fov_expected = fe; ctx_filled = 0; fov_filled = 0;
    ...
    if (state == RecvState::RECV_DATA) {
        ...
        if (kUseContext && line_buf[0] == 'C') {
            int off; char* csv = strchr(strchr(line_buf, ' ') + 1, ' ') + 1; sscanf(line_buf, "C %d", &off);
            if (kModelIOIsFloat) { int wrote = ParseFloatsIntoFloat(input_ctx_tensor->data.f, off, ctx_expected, csv); ctx_filled = std::min(ctx_expected, off + wrote); }
            else { int wrote = ParseIntsIntoInt8(input_ctx_tensor->data.int8, off, ctx_expected, csv); ctx_filled = std::min(ctx_expected, off + wrote); }
            SendAck("C", ctx_filled);
            return;
        }

        if (kUseFovea && line_buf[0] == 'F') {
            int off; char* csv = strchr(strchr(line_buf, ' ') + 1, ' ') + 1; sscanf(line_buf, "F %d", &off);
            if (kModelIOIsFloat) { int wrote = ParseFloatsIntoFloat(input_fov_tensor->data.f, off, fov_expected, csv); fov_filled = std::min(fov_expected, off + wrote); }
            else { int wrote = ParseIntsIntoInt8(input_fov_tensor->data.int8, off, fov_expected, csv); fov_filled = std::min(fov_expected, off + wrote); }
